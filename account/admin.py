...
class UserCreationForm(forms.ModelForm):
    ...
    password2 = forms.CharField(label=_(u"Confirme sua senha"), widget=forms.PasswordInput)

    class Meta:
        model = User
        fields = ('email', 'first_name', 'last_name', 'birthday', 'is_admin')

    def clean_password(self):
        password = self.cleaned_data.get("password")
        password2 = self.cleaned_data.get("password2")
        if password and password2 and password != password2:
            raise forms.ValidationError(u"As senhas informadas não são idênticas.")
        return password
...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ('email', 'first_name', 'last_name', 'birthday', 'is_admin',)
    ...
    add_fieldsets = (
        (None, {'classes': ('wide',),
                'fields': ('email', 'first_name', 'last_name', 'birthday', 'password', 'password2')}
         ),
    )
